...
@status_check
def gen_device_fts(spark, table_name, out_dir, run_date, **kwargs):
    
    print("generating fts for:", run_date)
    _start_date = datetime.strptime(run_date, "%Y%m%d") - relativedelta(months=3)
    _start_date = _start_date.strftime("%Y%m%d")
        
    query_1 = f'''
        select max(s3_file_date) max_date
        from {table_name} where s3_file_date >= '{_start_date}' and s3_file_date <= '{run_date}'
    '''
    ...
    # create fts
    device = device.withColumn("device_age", F.expr(f"{int(run_date[:4])} - Year_Released"))\
    ...
    # write to parquet
    utils.save_to_s3(device, f"{out_dir}/date={run_date}")
    # device.unpersist()
