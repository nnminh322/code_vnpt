...
def gen_bts_voice_lxw_fts(spark, bts_voice_dir, snapshot_str, level):
    ...
    for i in [1, 4, 12, 24]:
        ...
        if (i==1):
            if (level == "weekly"):
                lower_fts =fts_utils.load_from_s3(spark,bts_voice_dir + "/daily/total_features").where(f"date >= {date_from_str} and date < {date_to_str}")
                old_suffix = "daily"
                new_suffix = "l1w"
            else:
                continue
            ...
            if level != "weekly":
                lower_fts =fts_utils.load_from_s3(spark,bts_voice_dir + "/l1w").where(f"date > {date_from_str} and date <= {date_to_str}")
                old_suffix = "l1w"
                new_suffix = freq_str
            else:
                continue
        ...
        out_dir = bts_voice_dir + f"/{freq_str}/date={snapshot_str}"
        fts_utils.save_to_s3(lxw_fts, out_dir)


def merge_bts_voice_fts(spark, bts_voice_lxw_dir, snapshot_str):
   
    print("merging lxw fts for", snapshot_str)

    l1w =fts_utils.load_from_s3(spark,bts_voice_lxw_dir + "/l1w").where(f"date='{snapshot_str}'").drop("date")
    l4w =fts_utils.load_from_s3(spark,bts_voice_lxw_dir + "/l4w").where(f"date='{snapshot_str}'").drop("date")
    l12w =fts_utils.load_from_s3(spark,bts_voice_lxw_dir + "/l12w").where(f"date='{snapshot_str}'").drop("date")
    l24w =fts_utils.load_from_s3(spark,bts_voice_lxw_dir + "/l24w").where(f"date='{snapshot_str}'").drop("date")
...
#     df_fts = df_fts.select(*ft_cols)
     ...
     # write to parquet
    out_dir = f"{bts_voice_lxw_dir}/final_fts/date={snapshot_str}"
...
while start_date <= end_date:
    ...
    if start_date.weekday() == 0:
        gen_bts_voice_lxw_fts(spark, bts_voice_dir, run_date, level="weekly")
        gen_bts_voice_lxw_fts(spark, bts_voice_dir, run_date, level="lxw")
        merge_bts_voice_fts(spark, bts_voice_dir, run_date)
