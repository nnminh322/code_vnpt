...
def gen_pn_bts_usage_fts(spark, bts_usage_dir, pn_fts_dir, run_date, pn_list):
    _pn = ', '.join(f"'{str(x)}'" for x in pn_list)
    df_fts = fts_utils.load_from_s3(spark, bts_usage_dir + "/final_fts")\
    ...
    for freq_str in ['l1w', 'l4w', 'l12w', 'l24w']:
        df_fts = (
            df_fts.withColumn(f"pn_bts_usage_count_nt_ratio_{freq_str}", F.expr(f"pn_bts_usage_count_nt_{freq_str} / pn_bts_usage_count_{freq_str} "))\
                  .withColumn(f"pn_bts_usage_count_wk_ratio_{freq_str}", F.expr(f"pn_bts_usage_count_wk_{freq_str} / pn_bts_usage_count_{freq_str} "))
        )
    ...
    out_dir = f"{pn_fts_dir}/bts_usage/date={run_date}"
