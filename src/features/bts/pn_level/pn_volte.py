...
def gen_pn_bts_volte_fts(spark, bts_volte_dir, pn_fts_dir, run_date, pn_list):
    _pn = ', '.join(f"'{str(x)}'" for x in pn_list)
    df_fts = fts_utils.load_from_s3(spark, bts_volte_dir + "/final_fts")\
    ...
    for freq_str in ['l1w', 'l4w', 'l12w', 'l24w']:
        call_in_ratio_str = f"pn_bts_volte_call_in_count_{freq_str} / pn_bts_volte_total_call_count_{freq_str}"
        call_out_ratio_str = f"pn_bts_volte_call_out_count_{freq_str} / pn_bts_volte_total_call_count_{freq_str}"
        cf_ratio_str = f"pn_bts_volte_cf_count_{freq_str} / pn_bts_volte_total_call_count_{freq_str}"
        
        df_fts = (
            df_fts.withColumn(f"pn_bts_volte_call_in_ratio_{freq_str}", F.expr(call_in_ratio_str))\
                  .withColumn(f"pn_bts_volte_call_out_ratio_{freq_str}", F.expr(call_out_ratio_str))\
                  .withColumn(f"pn_bts_volte_cf_ratio_{freq_str}", F.expr(cf_ratio_str))
        )
    ...
    out_dir = f"{pn_fts_dir}/bts_volte/date={run_date}"
