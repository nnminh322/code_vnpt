...
def gen_pn_bts_ggsn_fts(spark, bts_ggsn_dir, pn_fts_dir, run_date, pn_list):
    _pn = ', '.join(f"'{str(x)}'" for x in pn_list)
    df_fts = fts_utils.load_from_s3(spark, bts_ggsn_dir + "/final_fts")\
    ...
    for freq_str in ['l1w', 'l4w', 'l12w', 'l24w']:
        over_5mins_ratio_str = f"pn_bts_ggsn_duration_count_over_5mins_{freq_str} / pn_bts_ggsn_duration_count_{freq_str}"
        under_30secs_ratio_str = f"pn_bts_ggsn_duration_count_under_30secs_{freq_str} / pn_bts_ggsn_duration_count_{freq_str}"

        df_fts = (
            df_fts.withColumn(f"pn_bts_ggsn_duration_count_ratio_over_5mins_{freq_str}", F.expr(over_5mins_ratio_str))\
                  .withColumn(f"pn_bts_ggsn_duration_count_ratio_under_30secs_{freq_str}", F.expr(under_30secs_ratio_str))
        )
    ...
    out_dir = f"{pn_fts_dir}/bts_ggsn/date={run_date}"
