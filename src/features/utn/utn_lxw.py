...
@status_check
def gen_utn_lxw_fts(spark, table_name, bt_table_name, bt_msisdn_column, out_dir, run_date, level, run_mode, common_config):
    ...
    for i in [1, 4, 12, 24]:
        ...
        query = f"""
            SELECT MSISDN as msisdn, CREDIT_AMOUNT, s3_file_date
            FROM {table_name}  
            WHERE 1=1
        ...
        if (i==1):
            if (level == "weekly"):
                ...
                df_fts = lower_fts.groupBy("msisdn").agg(
                    F.count("*").alias(f"utn_credit_count_{freq_str}"),
                    F.countDistinct("s3_file_date").alias(f"utn_count_days_{freq_str}"),
                    F.sum("CREDIT_AMOUNT").alias(f"utn_amount_sum_{freq_str}"),
                    F.avg("CREDIT_AMOUNT").alias(f"utn_amount_avg_{freq_str}"),
                    F.max("CREDIT_AMOUNT").alias(f"utn_amount_max_{freq_str}"),
                )
            ...
            if level != "weekly":
                ...
                utn_avg_str = f"utn_amount_sum_{freq_str}/utn_credit_count_{freq_str}"
                
                df_fts = lower_fts.groupBy("msisdn").agg(*aggs)\
                                  .withColumn(f"utn_amount_avg_{freq_str}", F.expr(utn_avg_str))
                
            else:
                continue
      
        # write to parquet
        utn_dir = out_dir + f"/{freq_str}/date={run_date}"
        utils.save_to_s3(df_fts, utn_dir)
        lower_fts.unpersist()


@status_check    
def merge_utn_fts(spark, out_dir, run_date, **kwargs):
    ...
    for ft in fts_names:
     ...
     # write to parquet
    utn_dir = f"{out_dir}/final_fts/date={run_date}"
    utils.save_to_s3(df_fts, utn_dir)
...
if __name__ == "__main__":
    ...
    run_create_feature(
        func_and_kwargs=(
            (gen_utn_lxw_fts, {'level':'weekly'}),
            (gen_utn_lxw_fts, {'level':'lxw'}),
            (merge_utn_fts, {})
        ),
        global_kwargs={
            'freq': 'weekly',
