...
@status_check
def gen_voice_lxw_fts(spark, out_dir, run_date, level):
...
@status_check
def merge_voice_fts(spark, out_dir, run_date):
    ...
    for freq_str in ['l1w', 'l4w', 'l12w', 'l24w']:
        call_in_ratio_str = f"voice_call_in_count_{freq_str} / voice_total_call_count_{freq_str}"
        call_out_ratio_str = f"voice_call_out_count_{freq_str} / voice_total_call_count_{freq_str}"

        call_not_answered_str = f"voice_total_call_not_answered_count_{freq_str} / voice_total_call_count_{freq_str}"

        sms_in_ratio_str = f"voice_sms_in_count_{freq_str} / voice_total_sms_count_{freq_str}"
        sms_out_ratio_str = f"voice_sms_out_count_{freq_str} / voice_total_sms_count_{freq_str}"

        total_call_ratio_str = f"voice_total_call_count_{freq_str} / voice_total_count_{freq_str}"
        total_sms_ratio_str = f"voice_total_sms_count_{freq_str} / voice_total_count_{freq_str}"


        
        df_fts = (
            df_fts.withColumn(f"voice_call_in_ratio_{freq_str}", F.expr(call_in_ratio_str))\
                  .withColumn(f"voice_call_out_ratio_{freq_str}", F.expr(call_out_ratio_str))\
                  .withColumn(f"voice_sms_in_ratio_{freq_str}", F.expr(sms_in_ratio_str))\
                  .withColumn(f"voice_sms_out_ratio_{freq_str}", F.expr(sms_out_ratio_str))\
                  .withColumn(f"voice_total_call_ratio_{freq_str}", F.expr(total_call_ratio_str))\
                  .withColumn(f"voice_total_sms_ratio_{freq_str}", F.expr(total_sms_ratio_str))\
                  .withColumn(f"voice_total_call_not_answered_ratio_{freq_str}", F.expr(call_not_answered_str))
        )
...
if __name__ == "__main__":
    ...
    run_create_feature(
        func_and_kwargs=(
            (gen_voice_lxw_fts, {'level':'weekly'}),
            (gen_voice_lxw_fts, {'level':'lxw'}),
            (merge_voice_fts, {})
        ),
        global_kwargs={
            'freq': 'weekly',
