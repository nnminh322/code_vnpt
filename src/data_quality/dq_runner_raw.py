...
def run_dq(common_config, dct_tables, layer='feature', to_edit_config=None):
    """
    ...
    # all_result = []
    # for t, info in dct_tables.items():
    #     table = to_edit_config['table_name'][t]
    #     destination = info['destination']
    #     dt_from = datetime.strptime(args['dt_from'], "%Y%m%d") + relativedelta(days=1)

    #     while dt_from <= dt_to:
    #         run_date = dt_from.strftime('%Y%m%d')
    #         if (int(run_date) >= 20250301 and t == 'blueinfo_voice_volte') or int(run_date) < 20250301 and t == 'blueinfo_voice_volte_v2':
    #             continue
    ...
    # dq_utils.trace_event(df_result, 'raw')
...
if __name__ == '__main__':
    ...
    run_dq(common_config, dct_tables, layer=layer_lv, to_edit_config = to_edit_config)