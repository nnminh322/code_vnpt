...
def cal_feature_by_date(DATE, fix_date_of_month): 
    ...
    df = (
        df
        .withColumn(f'{feature_col}', F.col(feature_col).cast(T.DoubleType()))
    ...
    df_agg_1 = (
        ...
        .groupBy(['msisdn'])
        .agg(
            F.sum(F.col(feature_col)).alias(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.avg(F.col(feature_col)).alias(f'{table_name}_avg_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.count(F.col(feature_col)).alias(f'{table_name}_count_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.min(F.col(feature_col)).alias(f'{table_name}_min_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.max(F.col(feature_col)).alias(f'{table_name}_max_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.stddev(F.col(feature_col)).alias(f'{table_name}_std_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.sum(F.col(feature_col+'_square')).alias(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'),
        )
    )
...
def cal_features_by_month(spark, table_name, feature_col, sub_col, fix_date_of_month):
    ...
    date_features_df.groupby("msisdn")
        .agg(
            F.sum(F.col(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=full'),
            F.sum(F.col(f'{table_name}_count_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_count_{feature_col}*{sub_col}*Filter=full'),
            (F.sum(F.col(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')) / F.sum(F.col(f'{table_name}_count_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}'))).alias(f'{table_name}_avg_{feature_col}*{sub_col}*Filter=full'),
            F.min(F.col(f'{table_name}_min_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_min_{feature_col}*{sub_col}*Filter=full'),
            F.max(F.col(f'{table_name}_max_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_max_{feature_col}*{sub_col}*Filter=full'),
            F.sum(F.col(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=full'),
        )
    )
...
def load_feature_by_month_and_merge(spark, table_name, feature_col, sub_col, ALL_MONTHS, fix_date):
    ...
    def remove_key_operator(feature_name):
    ...
    removed_key_operator_features = list(map(lambda x: remove_key_operator(x), selected_features))
