...
def cal_feature_by_date(spark, DATE , table_name, feature_col, sub_col, fix_date_of_month):
    ...
    # MOC    
    df1 = df.filter(df.CALL_TYPE == "MOC").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "MTC").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    ...
    # MTC
    df1 = df.filter(df.CALL_TYPE == "MOC").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "MTC").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    ...
    # ROA
    df1 = df.filter(df.CALL_TYPE == "ROA").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "ROA").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    ...
    # CF
    df1 = df.filter(df.CALL_TYPE == "CF").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "CF").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    ...
    # SMO
    df1 = df.filter(df.CALL_TYPE == "SMO").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "SMT").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    ...
    # SMT
    df1 = df.filter(df.CALL_TYPE == "SMO").select("PARTY_NUMBER", "CALL_DURATION").withColumnRenamed("PARTY_NUMBER", "msisdn")
    df2 = df.filter(df.CALL_TYPE == "SMT").select("MSISDN", "CALL_DURATION").withColumnRenamed("MSISDN", "msisdn")
    df_agg_6 = df1.union(df2)
    df_agg_6 = df_agg_6.withColumn("CALL_DURATION", df_agg_6['CALL_DURATION'].cast(T.DoubleType()))
    df_agg_6 = (df_agg_6.withColumn(f'{feature_col}_square', F.pow(F.col(feature_col).cast(T.DoubleType()), 2)))
    df_agg_6 = (
    ...
    filter_df = utils.spark_read_data_from_singlestore(spark,  f"SELECT DISTINCT (MSISDN) AS msisdn FROM {table_name_on_production} WHERE IMSI LIKE '45202%' AND s3_file_date = {DATE}")
