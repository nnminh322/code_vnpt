...
@F.udf(returnType = T.IntegerType())
def count_active_date(category):
    count = len(list(filter(lambda x: x != 'Y', category)))
    return count

def cal_features_by_month(spark, fix_date_of_month, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col):
    ...
    df_agg_1 = (
        df.groupBy(['msisdn']).agg(
            F.collect_list(feature_col_1).alias(f'{table_name}_list_{feature_col_1}*{sub_col}*Filter=full'),
        )
        .withColumn(f'{table_name}_frequentdict_{feature_col_1}*{sub_col}*Filter=full', get_frequent_dict_of_category(F.col(f'{table_name}_list_{feature_col_1}*{sub_col}*Filter=full')))
        .drop(f'{table_name}_list_{feature_col_1}*{sub_col}*Filter=full')
    )
    df_agg_2 = (
        df.groupBy(['msisdn']).agg(
            F.collect_list(feature_col_2).alias(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full'),
        )
        .withColumn(f'{table_name}_frequentdict_{feature_col_2}*{sub_col}*Filter=full', get_frequent_dict_of_category(F.col(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full')))
        .drop(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full')
    )
    df_agg_3 = (
        df.groupBy(['msisdn']).agg(
            F.collect_list(feature_col_3).alias(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full'),
        )
        .withColumn(f'{table_name}_count_{feature_col_3}*{sub_col}*Filter=full', count_active_date(F.col(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full')))
        .drop(f'{table_name}_list_{feature_col_2}*{sub_col}*Filter=full')
    )
    ...
    df_agg_joined_2 = (
        df_agg_joined_1.withColumnRenamed("msisdn", "msisdn_df1").join(df_agg_3.withColumnRenamed("msisdn", "msisdn_df2"), F.col('msisdn_df1') == F.col('msisdn_df2'), how ="fullouter")
        .withColumn('msisdn_new', F.coalesce("msisdn_df1", "msisdn_df2"))
        .drop('msisdn_df1', 'msisdn_df2')
        .withColumnRenamed("msisdn_new", "msisdn")
    )
    ...
    # save to s3
    print('Saving ...')
    utils.save_to_s3(df_agg_joined_2, table_name, f'/l1m/date={fix_date_of_month}', run_mode)
...
def run_all_month(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date):
    for MONTH in ALL_MONTHS:
        ...
        try:
            ...
            cal_features_by_month(spark, fix_date_of_month, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col)
        et = time.time()
...
def load_feature_by_month_and_merge(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date):
    ...
    for group in group_feature_by_indexes_level_2:
        ...
        # detect group type
        if len(list(filter(lambda x: 'sumsquare' in x , group_fetures[0]))) > 0:
            feature_type = 'numeric'
        elif len(list(filter(lambda x: 'frequentdict' in x , group_fetures[0]))) > 0:
        ...
        if feature_type == 'category':
            ...
            selected_df_with_label = (
                selected_df_with_label
                .withColumn(f'f_{table_name}_sum_{group_name}_T-1-acc', F.col(group_fetures[0][0]) )
                .withColumn(f'f_{table_name}_count_{group_name}_T-1-acc', F.col(group_fetures[0][2]) )
                .withColumn(f'f_{table_name}_avg_{group_name}_T-1-acc', F.col(group_fetures[0][1]) )
                .withColumn(f'f_{table_name}_min_{group_name}_T-1-acc', F.col(group_fetures[0][3]) )
                .withColumn(f'f_{table_name}_max_{group_name}_T-1-acc', F.col(group_fetures[0][4]) )
                .withColumn(f'f_{table_name}_sumsquare_{group_name}_T-1-acc', F.col(group_fetures[0][6]) )
            ...
            for i, month in enumerate(acc_months[1:]):
                selected_df_with_label = (
                    selected_df_with_label
                    .withColumn(f'f_{table_name}_sum_{group_name}_{month}-acc', F.col(group_fetures[i+1][0]) + F.col(f'f_{table_name}_sum_{group_name}_{acc_months[i]}-acc') )
                    .withColumn(f'f_{table_name}_count_{group_name}_{month}-acc',  F.col(group_fetures[i+1][2]) + F.col(f'f_{table_name}_count_{group_name}_{acc_months[i]}-acc') )
                    ...
                    .withColumn(f'f_{table_name}_min_{group_name}_{month}-acc', F.least( F.col(group_fetures[i+1][3]), F.col(f'f_{table_name}_min_{group_name}_{acc_months[i]}-acc') )  )
                    .withColumn(f'f_{table_name}_max_{group_name}_{month}-acc', F.greatest( F.col(group_fetures[i+1][4]), F.col(f'f_{table_name}_max_{group_name}_{acc_months[i]}-acc') ) )
                    .withColumn(f'f_{table_name}_sumsquare_{group_name}_{month}-acc', F.col(group_fetures[i+1][6]) + F.col(f'f_{table_name}_sumsquare_{group_name}_{acc_months[i]}-acc') )
    ...
    filter_acc_features = []
    category_operators = ['_least-frequent-item_', '_most-frequent-item_', '_unique_', '_most-frequent-value_', '_least-frequent-value_', '_most-frequent-percent_', '_least-frequent-percent_' ]
    numeric_operators = ['_sum_', '_count_', '_avg_', '_min_', '_max_', '_std_', '_ratio_' ]
    other_operators = []
    
    for column in selected_df_with_label.columns:
    
        is_operator = (len(list(filter(lambda x: x in column , category_operators +numeric_operators + other_operators))) >= 1)
        if (column.startswith('f_') and column.endswith('-acc') and is_operator) or column in ['date', 'msisdn']:
            filter_acc_features.append(column)
    ...
    for group in group_names:
        ...
        for operator in fill_na_stratery[group]:
...
feature_col_1 = 'last_action'
feature_col_2 = 'service_code'
feature_col_3 = 'active'
sub_col = ''
