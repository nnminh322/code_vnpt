...
# calculate by date
def cal_feature_by_date(DATE, fix_date_of_month):
    n_partitions = spark.sparkContext.defaultParallelism * 2
    
    ### get data
    print('Get data ...')
    if run_mode == "prod":
        query = f"select MSISDN, TOTALCOST, CHARGINGCATEGORYNAME from {table_name_on_production} where s3_file_date ='{DATE}'"
    else:
        query = f"select MSISDN, TOTALCOST, CHARGINGCATEGORYNAME from {table_name_on_production} where s3_file_date ='{DATE}' and MSISDN in (select {backtest_table_phone_name} from {backtest_table_name})"
    df = utils.spark_read_data_from_singlestore(spark, query)
    df = df.repartition(n_partitions)
    df = (
        df
        .withColumn('msisdn', F.col('MSISDN'))
        .withColumn('totalcost', F.col('TOTALCOST').cast(T.FloatType()))
        .withColumn(f'{feature_col}_square', F.pow(F.col(feature_col).cast(T.DoubleType()), 2))
    )
    
    aggregation = [] 
    for category in charging_cate_names: 
        condition_expr = f"CASE WHEN CHARGINGCATEGORYNAME = '{category}' THEN {feature_col} ELSE NULL END"
        condition_expr_2 = f"CASE WHEN CHARGINGCATEGORYNAME = '{category}' THEN {feature_col}_square ELSE NULL END" # square column

        aggregation.extend([
                F.sum(F.expr(condition_expr)).alias(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                (F.sum(F.expr(condition_expr))/F.count(F.expr(condition_expr))).alias(f'{table_name}_avg_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                F.count(F.expr(condition_expr)).alias(f'{table_name}_count_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                F.min(F.expr(condition_expr)).alias(f'{table_name}_min_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                F.max(F.expr(condition_expr)).alias(f'{table_name}_max_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                F.stddev(F.expr(condition_expr)).alias(f'{table_name}_std_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
                F.sum(F.expr(condition_expr_2)).alias(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'),
            ]
        )
        
    df_agg_1 = df.groupBy('msisdn').agg(*aggregation)
    
    #### SAVE S3
    print('Save ...')
    utils.save_to_s3_by_date(df_agg_1, table_name, DATE, fix_date_of_month, run_mode)
    spark.sparkContext._jvm.System.gc()
    
    ### FINISH
    print(f'FINISH {DATE}')

def cal_features_by_month(fix_date_of_month, feature_col, sub_col):
    ...
    dates_in_month = date_in_month.toPandas()['s3_file_date'].to_list()
    ...
    for i, date in enumerate(dates_in_month):
        ...
        try:
            st = time.time()
            DATE = date
            cal_feature_by_date(DATE, fix_date_of_month)    
            et = time.time()
            ...
            try:                
                ...
                et = time.time()
                print(f'Time taked: {str((et - st) /60)} minutes')
        date_finished.append(date)    ##### add check
...
#     print('==> Current number of partitions:', date_features_df.rdd.getNumPartitions())
    ...
    for category in charging_cate_names:
        aggregation.extend(
            [
                F.sum(F.col(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}')).alias(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}'),
                F.sum(F.col(f'{table_name}_count_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}')).alias(f'{table_name}_count_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}'),
                (F.sum(F.col(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'))/F.sum(F.col(f'{table_name}_count_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}'))).alias(f'{table_name}_avg_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}'),
                F.min(F.col(f'{table_name}_min_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}')).alias(f'{table_name}_min_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}'),
                F.max(F.col(f'{table_name}_max_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}')).alias(f'{table_name}_max_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}'),
                F.sum(F.col(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}_{fix_date_of_month}')).alias(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=CHARGINGCATEGORYNAME=={category}')
            ]
        )
    
    final_df = date_features_df.groupBy("msisdn").agg(*aggregation)
    
    #### SAVE S3
    print('Save ...')
    utils.save_to_s3(final_df, table_name, f'/l1m/date={fix_date_of_month}', run_mode)  
...
def run_all_month(spark, table_name, feature_col, sub_col, ALL_MONTHS, fix_date):
    for MONTH in ALL_MONTHS:
        ...
        try:
            ...
            cal_features_by_month(fix_date_of_month, feature_col, sub_col)
        et = time.time()
        print(f'Time taked: {str((et - st)/60)} minutes')
        print('')
...
feature_col = 'totalcost'
sub_col = ''
charging_cate_names = ['', 'DATA', 'VoLTE', 'SMS', 'VOICE', 'SERVICE_CHARGING (SCAP)', 'ROAMING', 'VoWIFI', 'MMS', 'IMS', 'VIDEO']
date_finished = []
