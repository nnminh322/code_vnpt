...
def cal_features_by_month(spark, fix_date_of_month, table_name, feature_col_1, feature_col_2, sub_col):
    print(f"------------------------{fix_date_of_month}---------------------------")
    
    # choose date by fix date of month
    n_partitions = spark.sparkContext.defaultParallelism * 2

    if fix_date_of_month[-2:] == '01':
        fix_date_of_month_time = datetime.strptime(fix_date_of_month, "%Y%m%d") - relativedelta(months=1)
        month_str = str(fix_date_of_month_time.year) +  str(f"{(fix_date_of_month_time.month):02d}")  
    elif fix_date_of_month[-2:] == '15':
        fix_date_of_month_time = datetime.strptime(fix_date_of_month, "%Y%m%d")
        month_str = str(fix_date_of_month_time.year) +  str(f"{(fix_date_of_month_time.month):02d}") 
    else:
        print('error')
        exit
    ...
    print('Getting data ...')
    if run_mode == 'prod':
        query = f"select * from {table_name_on_production} where s3_file_date = {month_str}"
    else: 
        query = f"select * from {table_name_on_production} where s3_file_date = {month_str} and MA_TB in (select {backtest_table_phone_name} from {backtest_table_name})"
    df = utils.spark_read_data_from_singlestore(spark, query)
    df = df.repartition(n_partitions)
    df = (
        df
        .withColumn('msisdn', F.col('MA_TB'))
        .withColumn(f'{feature_col_1}', F.col(feature_col_1).cast(T.DoubleType()))
        .withColumn(f'{feature_col_2}', F.col(feature_col_2).cast(T.DoubleType()))
        .withColumn(f'{feature_col_1}_square', F.pow(F.col(feature_col_1).cast(T.DoubleType()), 2))
        .withColumn(f'{feature_col_2}_square', F.pow(F.col(feature_col_2).cast(T.DoubleType()), 2))
    )
    df_agg_1 = (
        df.groupBy(['msisdn']).agg(
            F.sum(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col_1}*{sub_col}*Filter=full'),
            F.avg(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_avg_{feature_col_1}*{sub_col}*Filter=full'),
            F.count(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col_1}*{sub_col}*Filter=full'),
            F.min(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col_1}*{sub_col}*Filter=full'),
            F.max(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col_1}*{sub_col}*Filter=full'),
            F.stddev(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col_1}*{sub_col}*Filter=full'),
            F.sum(F.col(feature_col_1+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col_1}*{sub_col}*Filter=full'),
        )
    )
    df_agg_2 = (
        df.groupBy(['msisdn']).agg(
            F.sum(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col_2}*{sub_col}*Filter=full'),
            F.avg(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_avg_{feature_col_2}*{sub_col}*Filter=full'),
            F.count(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col_2}*{sub_col}*Filter=full'),
            F.min(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col_2}*{sub_col}*Filter=full'),
            F.max(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col_2}*{sub_col}*Filter=full'),
            F.stddev(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col_2}*{sub_col}*Filter=full'),
            F.sum(F.col(feature_col_2+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col_2}*{sub_col}*Filter=full'),
        )
    )
    
    # Perform join
    print('Performing join ...')
    df_agg_joined_1 = df_agg_1.join(df_agg_2, 'msisdn', how ="fullouter")
...
# run by month
def run_all_month(spark, table_name, feature_col_1, feature_col_2, sub_col, ALL_MONTHS, fix_date):
    for MONTH in ALL_MONTHS:
        ...
        try:
            ...
            cal_features_by_month(spark, fix_date_of_month, table_name, feature_col_1, feature_col_2, sub_col)
        et = time.time()
...
def load_feature_by_month_and_merge(spark, table_name, ALL_MONTHS, fix_date):
    ...
    for group in group_feature_by_indexes_level_2:
        ...
        selected_df_with_label = (
            selected_df_with_label
            .withColumn(f'f_{table_name}_sum_{group_name}_T-3-acc', F.col(group_fetures[2][0]) )
            .withColumn(f'f_{table_name}_count_{group_name}_T-3-acc', F.col(group_fetures[2][2]) )
            .withColumn(f'f_{table_name}_avg_{group_name}_T-3-acc', F.col(group_fetures[2][1]) )
            .withColumn(f'f_{table_name}_min_{group_name}_T-3-acc', F.col(group_fetures[2][3]) )
            .withColumn(f'f_{table_name}_max_{group_name}_T-3-acc', F.col(group_fetures[2][4]) )
            .withColumn(f'f_{table_name}_sumsquare_{group_name}_T-3-acc', F.col(group_fetures[2][6]) )
            .withColumn(f'f_{table_name}_variance_{group_name}_T-3-acc', (F.col(f'f_{table_name}_sumsquare_{group_name}_T-3-acc') / F.col(f'f_{table_name}_count_{group_name}_T-3-acc')) - F.col(f'f_{table_name}_avg_{group_name}_T-3-acc') ** 2 )
            .withColumn(f'f_{table_name}_std_{group_name}_T-3-acc', F.sqrt(F.col(f'f_{table_name}_variance_{group_name}_T-3-acc')) )
        )
        for i, month in enumerate(acc_months[1:]):
            selected_df_with_label = (
                selected_df_with_label
                .withColumn(f'f_{table_name}_sum_{group_name}_{month}-acc', F.col(group_fetures[i+3][0]) + F.col(f'f_{table_name}_sum_{group_name}_{acc_months[i]}-acc') )
                .withColumn(f'f_{table_name}_count_{group_name}_{month}-acc',  F.col(group_fetures[i+3][2]) + F.col(f'f_{table_name}_count_{group_name}_{acc_months[i]}-acc') )
                ...
                .withColumn(f'f_{table_name}_min_{group_name}_{month}-acc', F.least( F.col(group_fetures[i+3][3]), F.col(f'f_{table_name}_min_{group_name}_{acc_months[i]}-acc') )  )
                .withColumn(f'f_{table_name}_max_{group_name}_{month}-acc', F.greatest( F.col(group_fetures[i+3][4]), F.col(f'f_{table_name}_max_{group_name}_{acc_months[i]}-acc') ) )
                .withColumn(f'f_{table_name}_sumsquare_{group_name}_{month}-acc', F.col(group_fetures[i+3][6]) + F.col(f'f_{table_name}_sumsquare_{group_name}_{acc_months[i]}-acc') )
                ...
                .withColumn(f'f_{table_name}_ratio_{group_name}_{month}-acc', F.col(f'f_{table_name}_sum_{group_name}_{month}-acc') / F.col(f'f_{table_name}_sum_{group_name}_T-3-acc') )
            )
...
feature_col_1 = 'TRAGOC'
feature_col_2 = 'TRATHUE'
sub_col = ''
date_finished = []
...
if run_mode == 'prod': 
    ...
    if is_table_finished:
        ...
        run_all_month(spark, table_name, feature_col_1, feature_col_2, sub_col, ALL_MONTHS, fix_date)
