...
def cal_feature_by_date(spark, DATE, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, fix_date_of_month):
    n_partitions = spark.sparkContext.defaultParallelism * 2
    print(f'Get data {DATE}')
    if run_mode == 'prod':
        query = f"select msisdn, DURATION, DATA_VOLUME_DOWNLINK, DATA_VOLUME_UPLINK, s3_file_date from {table_name_on_production} where s3_file_date = {DATE}"
    else:
        query = f"select msisdn, DURATION, DATA_VOLUME_DOWNLINK, DATA_VOLUME_UPLINK, s3_file_date from {backtest_table_name} where s3_file_date = {DATE}"
    ...
    df = (
        ...
        .drop('DURATION', 'DATA_VOLUME_DOWNLINK', 'DATA_VOLUME_UPLINK')
        .withColumn('DURATION_MINUTES_square', F.pow(F.col('DURATION_MINUTES'), 2).cast(T.DoubleType()))
        .withColumn('DATA_VOLUME_DOWNLINK_MB_square', F.pow(F.col('DATA_VOLUME_DOWNLINK_MB').cast(T.DoubleType()), 2))
        .withColumn('DATA_VOLUME_UPLINK_MB_square', F.pow(F.col('DATA_VOLUME_UPLINK_MB').cast(T.DoubleType()), 2))
    )

    ##  DURATION
    df_agg_1 = (
        df.groupBy(['msisdn']).agg(
            F.sum(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            (F.sum(F.col(feature_col_1)) / F.count(F.col(feature_col_1))).alias(f'{table_name}_avg_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.count(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.min(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.max(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.stddev(F.col(feature_col_1)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.sum(F.col(feature_col_1+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'),
        )
    )

    ## DATA_VOLUME_DOWNLINK_MB 
    df_agg_2 = (
        df.groupBy(['msisdn']).agg(
            F.sum(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            (F.sum(F.col(feature_col_2)) / F.count(F.col(feature_col_2))).cast(T.DoubleType()).alias(f'{table_name}_avg_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.count(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.min(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.max(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.stddev(F.col(feature_col_2)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.sum(F.col(feature_col_2+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'),
        )
    )
    ...
    df_agg_3 = (
        df.groupBy(['msisdn']).agg(
            F.sum(F.col(feature_col_3)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            (F.sum(F.col(feature_col_3)) / F.count(F.col(feature_col_3))).cast(T.DoubleType()).alias(f'{table_name}_avg_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.count(F.col(feature_col_3)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.min(F.col(feature_col_3)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.max(F.col(feature_col_3)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.stddev(F.col(feature_col_3)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
            F.sum(F.col(feature_col_3+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'),
        )
    )

    df_agg_joined = df_agg_1.join(df_agg_2, 'msisdn', how ="fullouter").join(df_agg_3, 'msisdn', how ="fullouter")
...
def cal_features_by_month(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, fix_date_of_month):    
    ...
    ### get data
    print('Get data ...')
    query = f"select distinct s3_file_date from {table_name_on_production} where s3_file_date BETWEEN '{start_date_str}' AND '{end_date_str}'  ORDER BY s3_file_date"
    ...
    for i, date in enumerate(dates_in_month):
        ...
        try:
            st = time.time()
            DATE = date
            cal_feature_by_date(spark, DATE, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, fix_date_of_month)   
            et = time.time()
    ...
    acc_date_features = (
        date_features_df.groupby("msisdn")
            .agg(
                F.sum(F.col(f'{table_name}_sum_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sum_{feature_col_1}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_count_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_count_{feature_col_1}*{sub_col}*Filter=full'),
                (F.sum(F.col(f'{table_name}_sum_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'))/F.sum(F.col(f'{table_name}_count_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}'))).alias(f'{table_name}_avg_{feature_col_1}*{sub_col}*Filter=full'),
                F.min(F.col(f'{table_name}_min_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_min_{feature_col_1}*{sub_col}*Filter=full'),
                F.max(F.col(f'{table_name}_max_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_max_{feature_col_1}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_sumsquare_{feature_col_1}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sumsquare_{feature_col_1}*{sub_col}*Filter=full'),
                
                F.sum(F.col(f'{table_name}_sum_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sum_{feature_col_2}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_count_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_count_{feature_col_2}*{sub_col}*Filter=full'),
                (F.sum(F.col(f'{table_name}_sum_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'))/F.sum(F.col(f'{table_name}_count_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}'))).alias(f'{table_name}_avg_{feature_col_2}*{sub_col}*Filter=full'),
                F.min(F.col(f'{table_name}_min_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_min_{feature_col_2}*{sub_col}*Filter=full'),
                F.max(F.col(f'{table_name}_max_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_max_{feature_col_2}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_sumsquare_{feature_col_2}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sumsquare_{feature_col_2}*{sub_col}*Filter=full'),
                
                F.sum(F.col(f'{table_name}_sum_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sum_{feature_col_3}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_count_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_count_{feature_col_3}*{sub_col}*Filter=full'),
                (F.sum(F.col(f'{table_name}_sum_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'))/F.sum(F.col(f'{table_name}_count_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}'))).alias(f'{table_name}_avg_{feature_col_3}*{sub_col}*Filter=full'),
                F.min(F.col(f'{table_name}_min_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_min_{feature_col_3}*{sub_col}*Filter=full'),
                F.max(F.col(f'{table_name}_max_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_max_{feature_col_3}*{sub_col}*Filter=full'),
                F.sum(F.col(f'{table_name}_sumsquare_{feature_col_3}*{sub_col}*Filter=full_{fix_date_of_month}')).alias(f'{table_name}_sumsquare_{feature_col_3}*{sub_col}*Filter=full')
            )
    )
...
def run_all_month(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date):
    date_finished = []
    for MONTH in ALL_MONTHS:
        ...
        try:
            ...
            cal_features_by_month(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, fix_date_of_month)
        et = time.time()
        print(f'Time taked: {str((et - st)/60)} minutes')
        print('')

def load_feature_by_month_and_merge(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date):
...
feature_col_1 = 'DURATION_MINUTES'
feature_col_2 = 'DATA_VOLUME_DOWNLINK_MB'
feature_col_3 = 'DATA_VOLUME_UPLINK_MB'
sub_col = ''
date_finished = []
...
if run_mode == 'prod': 
    ...
    if is_table_finished:
        ...
        run_all_month(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date)
        load_feature_by_month_and_merge(spark, table_name, feature_col_1, feature_col_2, feature_col_3, sub_col, ALL_MONTHS, fix_date)
else: 
