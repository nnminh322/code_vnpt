...
def cal_features_by_month(spark, table_name, fix_date_of_month, feature_col, sub_col):
    ...
    if run_mode == 'prod':
        ...
        query = f"select * from {backtest_table_name} where s3_file_date BETWEEN '{start_date_str}' AND '{end_date_str}'"
        
    df = utils.spark_read_data_from_singlestore(spark, query)
    df = df.withColumn('AMOUNT',df['AMOUNT'].cast(T.FloatType()))
    df = (
        df
    ...
    df_agg_1_new_columns = list(map(lambda x: x.replace(feature_col, feature_col + '_SEND') , df_agg_1.columns ))
    for f_old, f_new in zip(df_agg_1.columns, df_agg_1_new_columns):
        df_agg_1 = df_agg_1.withColumnRenamed(f_old, f_new)

    df_agg_2 = (
        df.groupBy(['RECEIVER']).agg(
            F.sum(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_sum_{feature_col}*{sub_col}*Filter=full'),
            F.avg(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_avg_{feature_col}*{sub_col}*Filter=full'),
            F.count(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_count_{feature_col}*{sub_col}*Filter=full'),
            F.min(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_min_{feature_col}*{sub_col}*Filter=full'),
            F.max(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_max_{feature_col}*{sub_col}*Filter=full'),
            F.stddev(F.col(feature_col)).cast(T.DoubleType()).alias(f'{table_name}_std_{feature_col}*{sub_col}*Filter=full'),
            # F.collect_list(feature_col).cast(T.DoubleType()).alias(f'{table_name}_list_{feature_col}_*{sub_col}*Filter=full')
            F.sum(F.col(feature_col+'_square')).cast(T.DoubleType()).alias(f'{table_name}_sumsquare_{feature_col}*{sub_col}*Filter=full')
        )
    ...
    df_agg_2_new_columns = list(map(lambda x: x.replace(feature_col, feature_col + '_RECEIVE') , df_agg_2.columns ))
    for f_old, f_new in zip(df_agg_2.columns, df_agg_2_new_columns):
        df_agg_2 = df_agg_2.withColumnRenamed(f_old, f_new)

    # Join
    print('Performing join ...')
    df_agg_joined_1 = (
        df_agg_1.withColumnRenamed("SENDER", "msisdn_df1").join(df_agg_2.withColumnRenamed("RECEIVER", "msisdn_df2"), F.col('msisdn_df1') == F.col('msisdn_df2'), how ="fullouter")
...
def run_all_month(spark, table_name, feature_col, sub_col, ALL_MONTHS, fix_date):
    for MONTH in ALL_MONTHS:
        ...
        try:
            ...
            cal_features_by_month(spark, table_name, fix_date_of_month, feature_col, sub_col)
        et = time.time()
